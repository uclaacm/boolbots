...
function Sidebar(props: SidebarProps): JSX.Element {
  ...
  const desc = [
    ...
    'Here we see some `print` statements. These print out the evaluation of the expression inside them. The expressions with the `!=` and `==` **operators** compare the values on the left and right of the operator. `!=` checks if they are not equal and `==` checks if they are. So, the first expression is saying "`num_of_antennas` is not equal to 1" which we can see is true!',
    'Comparison operators are used to create expressions that evaluate to a boolean by comparing the two sides of the expression. There are 6 comparison operators in Python: `!=`, `==`, `<`, `<=`, `>`, and `>=`. These correspond to "not equal", "equal", "less than", "less than or equal to", "greater than", and "greater than or equal to" respectively.',
    '**Boolean operators** (also called logical operators) act on boolean values or expressions to return a boolean value. The `and` operator returns true if both expressions are true and false otherwise. The `or` operator returns true if either expression is true and only returns false if both are false. The `not` operator returns the opposite of the expression it is with.',
